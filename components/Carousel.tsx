...
export default function Carousel({ slides }: { slides: slide[] }) {
  ...
  return (
    <div
      className='relative h-[calc(100vh-64px)] overflow-hidden border-y-[1px]
      border-cyan-400 bg-pink-100'
    >
      {/* Slides */}
      ...
      <button className='absolute left-4 top-1/2 -translate-y-1/2 transform focus:outline-none' onClick={prevSlide}>
        <ChevronDoubleLeftIcon className='h-10 w-10 rounded-lg bg-cyan-300 p-1 font-bold' />
      </button>
      <button className='absolute right-4 top-1/2 -translate-y-1/2 transform focus:outline-none' onClick={nextSlide}>
        <ChevronDoubleRightIcon className='h-10 w-10 rounded-lg bg-cyan-300 p-1 font-bold' />
      </button>
